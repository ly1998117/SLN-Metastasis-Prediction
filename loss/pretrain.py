...
class Contrast(torch.nn.Module):
    def __init__(self, local_rank, batch_size, temperature=0.5):
        super().__init__()
        device = torch.device(f"cuda:{local_rank}")
        self.batch_size = batch_size
        self.register_buffer("temp", torch.tensor(temperature).to(torch.device(f"cuda:{local_rank}")))
        self.register_buffer("neg_mask", (~torch.eye(batch_size * 2, batch_size * 2,
                                                     dtype=torch.bool).to(device)).float())
...
class Loss(torch.nn.Module):
    def __init__(self, batch_size, local_rank):
        ...
        self.contrast_loss = Contrast(local_rank=local_rank, batch_size=batch_size).cuda()
        self.alpha1 = 1.0
        self.alpha2 = 1.0
        self.alpha3 = 1.0
